...
if __name__ == "__main__":
 ...
 # nsteps      = int(sum(1 for line in open(from_path+'t_tot_s.txt'))/part_number)
  ...
  for i in range(np.size(from_path_list)):
      ...
      H[H<1e3] = np.nan
      img=ax.pcolormesh(Theta,  R,  H, norm=colors.LogNorm(vmin=1000, vmax=1e5), cmap=mycolor_rainbow)
    ...
    #plt.title('electron at y='+str(round(y[n,0]/2/np.pi,4)),fontdict=font)
      ...
      plt.subplots_adjust(top=0.90, bottom=0.11, left=0.1, right=0.95, hspace=0.10, wspace=0.05)
    
      fig = plt.gcf()
      fig.set_size_inches(7.4, 7.)
    #fig.set_size_inches(5, 4.5)
      fig.savefig(to_path+'theta_en_dist_'+to_path[7:14]+'.png',format='png',dpi=160)
      plt.close("all")
