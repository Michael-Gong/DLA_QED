...
if __name__ == "__main__":
  ...
  n=1
  plt.subplot(2,2,1)
  plt.scatter(x0[n,:], y0[n,:], c=gg0[n,:], norm=colors.Normalize(vmin=0,vmax=500), s=10, cmap='autumn', edgecolors='None', alpha=1,zorder=3)
  plt.plot(x0[n,:], y_max[n,:],'--k',linewidth=4.,label=r'$Reduced\ w=1$')
  plt.plot(x0[n,:], -y_max[n,:],'--k',linewidth=4.,label=r'$Reduced\ w=1$')
...
#  plt.contour(px, py, R, levels=levels, norm=mcolors.LogNorm(vmin=levels.min(), vmax=levels.max()), linestyles='dashed', cmap='copper',zorder=0)
  plt.contourf(X, Y, Ey, levels=levels, norm=mcolors.Normalize(vmin=levels.min(), vmax=levels.max()), cmap='PiYG_r',zorder=0,alpha=1)
  ...
  plt.xlim(0,20)
  plt.ylim(-10,10)
  plt.xticks([0,10,20],fontsize=font_size); plt.yticks([-10,-5,0,5,10],fontsize=font_size);
  #plt.xscale('log')
#  plt.xlim(-200,15200)
#  plt.ylim(-800,800)
...
#  plt.ylim(-800,800)
  ...
  plt.subplot(2,2,3)
  plt.scatter(x1[n,:], y1[n,:], c=gg1[n,:], norm=colors.Normalize(vmin=00,vmax=10000), s=10, cmap='winter', edgecolors='None', alpha=1,zorder=3)
  plt.plot(x1[n,:], y_max[n,:],'--k',linewidth=4.,label=r'$Reduced\ w=1$',zorder=4)
  plt.plot(x1[n,:], -y_max[n,:],'--k',linewidth=4.,label=r'$Reduced\ w=1$',zorder=5)
  condition = ((radn1[n,1:]-radn1[n,:-1]) > 0) & ((radt1[n,1:]-radt1[n,:-1]) > 50.0 )
  plt.scatter(x1[n,:-1][condition], y1[n,:-1][condition], c=(radt1[n,1:]-radt1[n,:-1])[condition]*0.51, norm=colors.Normalize(vmin=50,vmax=500), s=150, cmap='hot', marker='*', edgecolors='k', alpha=1,zorder=6)
...
#  print(np.min(R),np.max(R))
#  plt.contour(px, py, R, levels=levels, norm=mcolors.LogNorm(vmin=levels.min(), vmax=levels.max()), linestyles='dashed', cmap='copper',zorder=0)
  plt.contourf(X, Y, Ey, levels=levels, norm=mcolors.Normalize(vmin=levels.min(), vmax=levels.max()), cmap='PiYG_r',zorder=1,alpha=1)
  cbar=plt.colorbar(pad=0.03, ticks=np.linspace(-1,1,3))#,orientation="horizontal")
  cbar.ax.set_yticklabels(cbar.ax.get_yticklabels(), fontsize=font_size)
  cbar.set_label('$E_y/E_0$', fontdict=font)
  plt.xlabel('X [$\mu m$]',fontdict=font)
...
#  plt.axis('off')
  ...
  fig.savefig('./wrap_x_y.png',format='png',dpi=160)
  plt.close("all")
