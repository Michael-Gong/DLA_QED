...
######################################################################
        insert1='./Nob'+str(int(round(nb0)))+'p'+str(int(round(np0)))+'/'
        #insert1='./Data/'
        print(insert1)
        nsteps = sum(1 for line in open(insert1+'x_0.txt'))
        ...
        t_no=np.reshape(t1,(part_number,nsteps))
        x_no=np.reshape(x1,(part_number,nsteps))
        y_no=np.reshape(y1,(part_number,nsteps))
        z_no=np.reshape(z1,(part_number,nsteps))
        px_no=np.reshape(px1,(part_number,nsteps))
        py_no=np.reshape(py1,(part_number,nsteps))
        pz_no=np.reshape(pz1,(part_number,nsteps))
        #ey=np.reshape(ey,(part_number,nsteps))
        #ay=np.reshape(ay,(part_number,nsteps))
        radn_no=np.reshape(radn1,(part_number,nsteps))
        radt_no=np.reshape(radt1,(part_number,nsteps))
        radpx_no=np.reshape(radpx1,(part_number,nsteps))
        opt_no=np.reshape(opt1,(part_number,nsteps))
        eta_no=np.reshape(eta1,(part_number,nsteps))
        
        gamma_no=np.sqrt(px_no**2+py_no**2+1)
        
        R_dep_no=gamma_no-px_no
        
        py_0_no = 1.00*np0
        alpha_0_no = 10**(nb0/1e4-3.5)
        R_max_no = py_0_no-(radt_no-radpx_no)
        C_1_no = (py_0_no**2+1.0)**0.5
        y_max0_no=(C_1_no/alpha_0_no)**0.5
        y_max_no=((C_1_no-(radt_no-radpx_no))/alpha_0_no)**0.5/2.0/np.pi
        
        index=0
        
        radn_x_no=(radn_no[index,1:]-radn_no[index,:-1])
        radt_x_no=(radt_no[index,1:]-radt_no[index,:-1])
        
        condition_no = np.where(radt_x_no>2)
#####################################################################
        ...
        plt.scatter(x_no[index,:]/2/np.pi, y_no[index,:]/2/np.pi, s=4, color='skyblue', edgecolors='None')
        ...
        plt.scatter(px_no[index,:],py_no[index,:], s=4, color='skyblue', edgecolors='None')
        ...
        plt.xticks(fontsize=15); plt.yticks(fontsize=30);
        ...
        plt.subplot(3,3,6)
        #plt.scatter((t[index,:]-x[index,:])/2/np.pi,gamma[index,:], c=eta[index,:], s=1, cmap='terrain', edgecolors='None')
        plt.scatter(x[index,:]/2/np.pi,gamma[index,:], c=eta[index,:], s=1, cmap='terrain', edgecolors='None')
        #plt.plot((t[index,:])/2/np.pi,np.sqrt(px[index,:]**2+py[index,:]**2+1),'--k',linewidth=2.5,label='No RR')
        #plt.legend(loc='upper right')
        cbar=plt.colorbar(ticks=np.linspace(np.min(eta[index,:]), np.max(eta[index,:]), 5))
        cbar.set_label(r'$\chi_e$', fontdict=font)#plt.xlim(47,53)
        plt.scatter(x_no[index,:]/2/np.pi,gamma_no[index,:], s=1, color='skyblue', edgecolors='None')
        #plt.ylim(-1.1,-0.7)
        plt.xlabel(r'$x [\mu m]$',fontdict=font)
        plt.ylabel(r'$\gamma$',fontdict=font)
        plt.xticks(fontsize=30); plt.yticks(fontsize=30);
        ...
        cbar=plt.colorbar(ticks=np.linspace(np.min(R_dep[index,:]), np.max(R_dep[index,:]), 5),shrink=1)
        cbar.set_label(r'$R$', fontdict=font)
        plt.scatter((t[index,condition]-x[index,condition])/2/np.pi,y[index,condition]/2/np.pi,c=(radt_x[condition])[np.newaxis,:], s=50, cmap='rainbow', edgecolors='None')
        plt.scatter((t_no[index,:]-x_no[index,:])/2/np.pi, y_no[index,:]/2/np.pi, s=4, color='skyblue', edgecolors='None')
        #cbar=plt.colorbar(ticks=np.linspace(np.min(radt_x[radn_x>0]), np.max(radt_x[radn_x>0]), 5), shrink=1)
        #cbar.set_label(r'$photon\ energy\ [m_ec^2]$', fontdict=font)
        #plt.xlim(47,53)
        #plt.ylim(-1.1,-0.7)
        ...
        plt.subplot(3,3,8)
        plt.scatter((t[index,:]-x[index,:])/2/np.pi, px[index,:]/2/np.pi, c=eta[index,:], s=1, cmap='terrain', edgecolors='None')
        #plt.scatter((t[index,:]-x[index,:])/2/np.pi, y_max, c=R_dep[index,:], s=1, cmap='rainbow', edgecolors='None')
        #plt.legend(loc='upper right')
        #plt.colorbar()
        cbar=plt.colorbar(ticks=np.linspace(np.min(R_dep[index,:]), np.max(R_dep[index,:]), 5),shrink=1)
        cbar.set_label(r'$\chi_e$', fontdict=font)
        plt.scatter((t_no[index,:]-x_no[index,:])/2/np.pi, px_no[index,:]/2/np.pi, s=1, color='skyblue', edgecolors='None')
        #plt.xlim(47,53)
        #plt.ylim(-1.1,-0.7)
        plt.xlabel(r'$\phi\ [2\pi]$',fontdict=font)
        ...
        plt.subplot(3,3,9)
        plt.scatter((t[index,:]-x[index,:])/2/np.pi, py[index,:]/2/np.pi, c=eta[index,:], s=1, cmap='terrain', edgecolors='None')
        #plt.scatter((t[index,:]-x[index,:])/2/np.pi, y_max, c=R_dep[index,:], s=1, cmap='rainbow', edgecolors='None')
        #plt.legend(loc='upper right')
        #plt.colorbar()
        cbar=plt.colorbar(ticks=np.linspace(np.min(R_dep[index,:]), np.max(R_dep[index,:]), 5),shrink=1)
        cbar.set_label(r'$\chi_e$', fontdict=font)
        plt.scatter((t_no[index,:]-x_no[index,:])/2/np.pi, py_no[index,:]/2/np.pi, s=1, color='skyblue', edgecolors='None')
        #plt.xlim(47,53)
        #plt.ylim(-1.1,-0.7)
        plt.xlabel(r'$\phi\ [2\pi]$',fontdict=font)
