...
if __name__ == "__main__":
  ...
  levels = np.linspace(0, 4e3, 41)
  plt.pcolormesh(x, y, data_no.T, norm=colors.Normalize(vmin=0, vmax=4e3), cmap=mycolor_jet)
...
#plt.title('electron at y='+str(round(y[n,0]/2/np.pi,4)),fontdict=font)
  ...
  levels = np.linspace(0, 4e3, 41)
  plt.pcolormesh(x, y, data_rr.T, norm=colors.Normalize(vmin=0, vmax=4e3), cmap=mycolor_jet)
  #plt.pcolormesh(x, y, data_rr.T, norm=colors.LogNorm(vmin=1, vmax=4e3), cmap=mycolor_jet)
...
#  alpha=np.linspace(-3.5,0.5,501)
  ...
  plt.ylabel('time [$T_0$]',fontdict=font)
  plt.xticks([0,1000,2000,3000,4000],fontsize=25); plt.yticks(fontsize=25);
  plt.title('dN/dE for ll RR', fontsize=25)
  plt.xlim(0,4000)
  plt.ylim(0,1000)
#plt.title('electron at y='+str(round(y[n,0]/2/np.pi,4)),fontdict=font)
  ...
  x,y=np.meshgrid(axis_gg,axis_time)
  levels = np.linspace(0, 4e3, 41)
  plt.pcolormesh(x, y, data_qe.T, norm=colors.Normalize(vmin=0, vmax=4e3), cmap=mycolor_jet)
  plt.axis([x.min(), x.max(), y.min(), y.max()])
#### manifesting colorbar, changing label and axis properties ####
  cbar=plt.colorbar(pad=0.003)#ticks=[np.min(ex), -eee/2, 0, eee/2, np.min()])
  cbar.ax.set_yticklabels(cbar.ax.get_yticklabels(), fontsize=20)
...
#plt.title('electron at y='+str(round(y[n,0]/2/np.pi,4)),fontdict=font)
  ...
  plt.subplots_adjust(top=0.92, bottom=0.12, left=0.1, right=0.95, hspace=0.10, wspace=0.15)

  fig = plt.gcf()
  fig.set_size_inches(30, 7.5)
#fig.set_size_inches(5, 4.5)
  fig.savefig('./jpg/time_integral_s_lin.png',format='png',dpi=160)
  plt.close("all")
