...
theta_grid = np.linspace(-180.0, 180.0, 91)
energy_grid= np.linspace(2,4002,101)

x_grid = np.linspace(-180.0, 180.0, 90)
y_grid = np.linspace(2,2002,100)
...
for index in range(part_number):
    ...
    print('finish '+str(index)+'%; max value is'+str(np.max(theta_energy)))

theta_energy = np.log10(theta_energy+1)

ax=plt.subplot(1,1,1)
levels = np.linspace(np.min(theta_energy), np.max(theta_energy), 12)
...
cbar.set_label(r'$log_{10}\frac{dN}{dEd\theta}$', fontdict=font)#plt.xlim(47,53)
