...
#  cbar.set_label('$\gamma$',fontdict=font)
  ...
  plt.xticks([0,500,1000,1500],fontsize=font_size); plt.yticks([-200,0,200],fontsize=font_size);
  #plt.xscale('log')
  plt.xlim(-200,1800)
  plt.ylim(-320,320)
  ...
  plt.subplot(1,2,2)
  plt.scatter(px1[n,t1[n,:]*10./3.<1000], py1[n,t1[n,:]*10./3.<1000], c=t1[n,t1[n,:]*10./3.<1000]*10./3., norm=colors.Normalize(vmin=0,vmax=1000), s=8, cmap='rainbow', edgecolors='None', alpha=1,zorder=1)
  #cbar=plt.colorbar( ticks=np.linspace(0, 12000, 3) ,pad=0.005)
  #cbar.ax.set_yticklabels(cbar.ax.get_yticklabels(), fontsize=font_size)
  #cbar.set_label('$\gamma$',fontdict=font)
  condition = ((radn1[n,1:]-radn1[n,:-1]) > 0) & ((radt1[n,1:]-radt1[n,:-1]) > 5.0 )
  plt.scatter(px1[n,:-1][condition], py1[n,:-1][condition], c=(radt1[n,1:]-radt1[n,:-1])[condition]*0.51, norm=colors.Normalize(vmin=50,vmax=500), s=150, cmap='hot', marker='*', edgecolors='k', alpha=1,zorder=2)
...
#  cbar.set_label('$\gamma$',fontdict=font)
  ...
  plt.xlabel('$p_x$ [$m_ec$]',fontdict=font)
  plt.ylabel('$p_y$ [$m_ec$]',fontdict=font)
  plt.xticks([0,500,1000,1500],fontsize=font_size); 
  plt.yticks([-200,0,200],fontsize=font_size);
  #plt.xscale('log')
  plt.xlim(-200,1800)
  plt.ylim(-320,320)
  plt.subplots_adjust(left=0.12, bottom=0.16, right=0.99, top=0.99,
  ...
  fig.savefig('./wrap_px_py_insert_no.png',format='png',dpi=160)
  plt.close("all")
