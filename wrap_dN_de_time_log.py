...
if __name__ == "__main__":
  ...
  data_no   = np.zeros([200,251])
  data_rr   = np.zeros([200,251])
  data_qe   = np.zeros([200,251])
  ...
  plt.pcolormesh(x*0.51*1e-3, y*10./3., data_no.T, norm=colors.LogNorm(vmin=1, vmax=1e4), cmap=mycolor_jet)
...
#  alpha=np.linspace(-3.5,0.5,501)
  ...
  plt.xticks([0,5,10],fontsize=font_size); plt.yticks([0,500,1000,1500],fontsize=font_size);
...
#plt.title('electron at y='+str(round(y[n,0]/2/np.pi,4)),fontdict=font)
  ...
  x,y=np.meshgrid(axis_gg,axis_time)
  levels = np.logspace(0, np.log10(1e4), 41)
  plt.pcolormesh(x*0.51*1e-3, y*10./3., data_qe.T, norm=colors.LogNorm(vmin=1, vmax=1e4), cmap=mycolor_jet)
#plt.pcolormesh(x, y, ex.T, norm=mpl.colors.Normalize(vmin=0,vmax=100,clip=True), cmap=cm.cubehelix_r)
  plt.axis([x.min(), x.max(), y.min(), y.max()])
#### manifesting colorbar, changing label and axis properties ####
#  cbar=plt.colorbar(pad=0.01)#ticks=[np.min(ex), -eee/2, 0, eee/2, np.min()])
...
#  alpha=np.linspace(-3.5,0.5,501)
  plt.xlabel(r'$\epsilon_e$'+' [GeV]',fontdict=font)
#  plt.ylabel('time [fs]',fontdict=font)
  plt.xticks([0,5,10],fontsize=font_size); 
  plt.yticks([0,500,1000,1500],fontsize=0.000001);
...
#plt.title('electron at y='+str(round(y[n,0]/2/np.pi,4)),fontdict=font)
  ...
  x,y=np.meshgrid(axis_gg,axis_time)
  levels = np.logspace(0, np.log10(1e4), 41)
  plt.pcolormesh(x*0.51*1e-3, y*10./3., data_rr.T, norm=colors.LogNorm(vmin=1, vmax=1e4), cmap=mycolor_jet)
#plt.pcolormesh(x, y, ex.T, norm=mpl.colors.Normalize(vmin=0,vmax=100,clip=True), cmap=cm.cubehelix_r)
  plt.axis([x.min(), x.max(), y.min(), y.max()])
#### manifesting colorbar, changing label and axis properties ####
#  cbar=plt.colorbar(pad=0.01)#ticks=[np.min(ex), -eee/2, 0, eee/2, np.min()])
...
#plt.title('electron at y='+str(round(y[n,0]/2/np.pi,4)),fontdict=font)
  ...
  plt.subplots_adjust(top=0.99, bottom=0.16, left=0.1, right=0.95, hspace=0.10, wspace=0.15)

  fig = plt.gcf()
  fig.set_size_inches(25, 6.2)
#fig.set_size_inches(5, 4.5)
  fig.savefig('./wrap_dN_dE_time.png',format='png',dpi=160)
  plt.close("all")
