...
if __name__ == "__main__":
  ...
  n=1
  plt.scatter(px0[n,:], py0[n,:], c=R0[n,:], norm=colors.Normalize(vmin=0,vmax=100), s=10, cmap='winter_r', edgecolors='None', alpha=1,zorder=3)
  print('py,',np.min(py0[n,:]),np.max(py0[n,:]))



  from_path = './Data_artificial/'
#  to_path   = './jpg_qe_part01/'
  ...
  plt.scatter(px1[n,:], py1[n,:], c=R1[n,:], norm=colors.Normalize(vmin=0,vmax=100), s=10, cmap='winter_r', edgecolors='None', alpha=1,zorder=3)
  print('py,',np.min(py1[n,:]),np.max(py1[n,:]))
  condition = ((radn1[n,1:]-radn1[n,:-1]) > 0) & ((radt1[n,1:]-radt1[n,:-1]) > 50.0 )
  plt.scatter(px1[n,:-1][condition], py1[n,:-1][condition], c=(radt1[n,1:]-radt1[n,:-1])[condition]*0.51, norm=colors.Normalize(vmin=50,vmax=500), s=300, cmap='Reds_r', marker='*', edgecolors='k', alpha=1,zorder=4)
  plt.scatter(px1[n,1:][condition], py1[n,1:][condition], c=(radt1[n,1:]-radt1[n,:-1])[condition]*0.51, norm=colors.Normalize(vmin=50,vmax=500), s=100, cmap='Reds_r', marker='o', edgecolors='k', alpha=1,zorder=4)
  print('photon gamma factor:',np.max((radt1[n,1:]-radt1[n,:-1])[condition]))
  cbar=plt.colorbar( ticks=np.linspace(0, 500, 6) ,pad=0.02)
  cbar.ax.set_yticklabels(cbar.ax.get_yticklabels(), fontsize=font_size)
  cbar.set_label('$E_{ph}$ [MeV]',fontdict=font)


  px = np.linspace(-400,4400,501)
  py = np.linspace(-350,350,401)

  px, py = np.meshgrid(px, py)
  gg = (1+px**2+py**2)**0.5
  R = (1+px**2+py**2)**0.5-px
  R[R<0.1]=0.1
  levels = np.linspace(0,4400,101)
  print(np.min(R),np.max(R))
  plt.contourf(px, py, gg, levels=levels, norm=mcolors.Normalize(vmin=levels.min(), vmax=levels.max()), cmap=mycolor_Reds,zorder=0)
  print('electron energy',np.max(gg))
  levels = np.logspace(1,3,3)
  plt.contour(px, py, R, levels=levels, colors='k', linestyles='dashed', zorder=1)
  #plt.contour(px, py, R, levels=levels, norm=mcolors.LogNorm(vmin=levels.min(), vmax=levels.max()), linestyles='dashed', cmap='gist_gray',zorder=1)
  #plt.contourf(px, py, R, levels=levels, norm=mcolors.LogNorm(vmin=levels.min(), vmax=levels.max()), cmap=mycolor_jet)
  cbar=plt.colorbar(pad=0.02, ticks=np.logspace(1,3,3))#,orientation="horizontal")
  cbar.ax.set_yticklabels(cbar.ax.get_yticklabels(), fontsize=font_size)
...
#  cbar.set_label('$\gamma$',fontdict=font)
  ...
  plt.xticks([0,1000,2000,3000,4000],fontsize=font_size); plt.yticks([-300,-150,0,150,300],fontsize=font_size);
  #plt.xscale('log')
  plt.xlim(-700,4200)
  plt.ylim(-320,320)
  ...
  fig.savefig('./px_py_artificial.png',format='png',dpi=160)
  plt.close("all")
